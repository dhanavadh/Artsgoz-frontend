            ...
            <div className="flex gap-10 border-b mb-6 max-md:px-4 border-grey pb-6 items-center">
                ...
                <div className="flex flex-col justify-between py-2 w-full min-w-[300px]">
                    <div>
                        ...
                        <p className="line-clamp-1">เผยแพร่เมือ {getDay(publishedAt)}</p>
                    </div>

                    <div className="flex gap-6 mt-3">
                        <Link to={`/editor/${blog_id}`} className="pr-4 py-2 underline">แก้ไข</Link>

                        <button className="lg:hidden pr-4 py-2 underline" onClick={() => setShowStat(preVal => !preVal)}>สถิติ</button>

                        <button className="pr-4 py-2 underline text-red" onClick={(e) => deleteBlog(blog, access_token, e.target)}>ลบ</button>
                    </div>
                </div>
